(function (root) {
  ...
  function isJobDismissLabel(label) {
    // LinkedIn formats a job card's dismiss control as "Dismiss <title> job".
    // Non-job interactive cards that LinkedIn interleaves into the results column
    // (promoted feed posts, suggestions, surveys) also expose a "Dismiss ..." button
    // but their label does NOT end in "job". Clicking one of those navigates the tab
    // away to /feed/update/... and unloads the content script, so they must be
    // excluded from the cards the scraper clicks.
    const cleaned = (label || "")
      .replace(/\s*\(Verified job\)\s*$/i, "")
      .trim();

    return /\bjob$/i.test(cleaned);
  }

  function cardResolvesToJobId(card) {
    const stableJobId = (
    ...
    return Boolean(stableJobId);
  }

  function isJobCard(card) {
    if (!card || typeof card.querySelector !== "function") {
      return false;
    }

    const dismissButton = card.querySelector(RESULTS_DISMISS_SELECTOR);
    if (!dismissButton) {
      return false;
    }
    ...
    return isJobDismissLabel(dismissLabel) || cardResolvesToJobId(card);
  }
  ...
  function findJobListContainer(doc = document) {
    ...
    const hasJobCard = findAll(list, RESULTS_CARD_SELECTOR).some(isJobCard);

    return hasJobCard ? list : null;
  }
  ...
  function getJobCards(rootNode = document) {
    ...
    return findAll(container, RESULTS_CARD_SELECTOR).filter(isJobCard);
  }
