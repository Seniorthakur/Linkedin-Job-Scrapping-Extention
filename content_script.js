...
if (!shouldBootstrapContentScript(window.__linkedInScraperLoaded, currentRuntimeId)) {
  ...
  //
  // The content script only runs on https://www.linkedin.com/jobs/*. If a click
  // during an active run navigates the tab anywhere else (most commonly a promoted
  // feed post at /feed/update/... that LinkedIn interleaves into the results list),
  // the script is unloaded and the run silently dies. While a run is active we block
  // any click that resolves to a link leaving the /jobs/ area. Legitimate scraper
  // actions (selecting a card, paging, expanding sections) act on <div role="button">
  // / <button> elements and same-page pushState navigation, so they are unaffected.
  // The listener is registered on window in the capture phase, so it runs at the very
  // top of the capture path, before any anchor's default navigation.
  window.addEventListener(
    ...
    (event) => {
      ...
      if (leavesJobsArea) {
        ...
        console.warn(
          "[LinkedInScraper] Blocked navigation away from job results during run:",
          destination.href
        );
      }
    },
  ...
  async function scrapeCurrentPage(cards, pageNum, startIndex) {
    for (let cardIndex = startIndex; cardIndex < cards.length; cardIndex += 1) {
      ...
      // Final safety check: never click a card that is not a real job posting.
      // collectPageCards already filters these out, but the results list can mutate
      // between collection and this click, and clicking a stray promoted/feed card
      // would navigate the tab off the jobs page and kill the run.
      if (!isJobCard(card)) {
        console.warn(`[LinkedInScraper] [${cardIndex + 1}/${cards.length}] Skipping non-job card`);
        continue;
      }
      ...
      // If the click moved the tab out of the /jobs/ area (e.g. an unexpected
      // router link inside a card), stop the run cleanly instead of extracting
      // against an unrelated page.
      if (!window.location.pathname.startsWith("/jobs/")) {
        console.error("[LinkedInScraper] Navigation left the job results area after a card click — stopping run.");
        setRunError("Left the job results page unexpectedly. Run stopped.");
        return "error";
      }
